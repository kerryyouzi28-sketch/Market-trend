...
@st.cache_data(ttl=300) # 快取數據 5 分鐘，避免頻繁請求 yfinance
def fetch_data():
    tickers = list(SECTOR_MAP.keys())
    data = yf.download(tickers, period="1m", interval="1d", progress=False)
    return data
...
try:
    ...
    with col3:
        st.metric("📈 加權大盤 3日變動", "加權指數", f"{round(market_3d, 2)}%")
